...
# Standardized Category Mapping (aligned with UI tabs)
CAT_MAP = {
    '실손보험': '실손보험', '암보험': '암보험', 
...
def standard_upload():
    ...
    # Use UPSERT via POST Prefer: resolution=merge-duplicates header
    ...
    for f in files:
        try:
            with open(os.path.join(D, f), 'r', encoding='utf-8') as jf:
                ...
                # Standardize category based on the map
                raw_cat = d.get('category', '보험제도/상식')
                ui_cat = CAT_MAP.get(raw_cat, '전체')
                
                t = d.get('title', f.replace('.json', '').replace('+', ' ').strip())
                
                # Unique ID consistent with sync_upload (using filename only)
    ...
    if recs:
        ...
        for i in range(0, len(recs), 50):
            ...
            print(f'Batch Upload Success: {i+1} to {min(i+50, len(recs))} (Status: {r.status_code})')
        print(f'Sync Complete! {len(recs)} standardized records uploaded.')
